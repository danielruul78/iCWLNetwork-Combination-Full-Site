<?php
	//-----------------------------------------------------------------------------------------------------------
	/*
	print("--Current Include File-".$_SERVER['PHP_SELF']."--\n\n");
	print_r($current_dir);
	 $file =$current_dir['dirname']."/".$current_dir['basename'];
    ...
    $ret_str="Invalid DB Include";
    foreach ($lines as $line_num => $line) {
       echo "Line #<b>{$line_num}</b> : " . htmlspecialchars($line) . "<br />\n";
       $pos = strpos($line, "include_once('clsDataBase.php');");
       if($pos>-1){
           $ret_str="valid DB Include";
       }
    }
    print $ret_str;
    */
				...
				*/
				...
				if($this->current_db_type=="MySQL"){
					if(isset($link->connect_error)) {
						$this->log->general("Connection failed: " . $link->connect_error,3);
					}else{
