...
class clsVariables
{
	...
	public function Look_GUID(){
		//$log->general("Start Look GUID".$this->guid,1);
		if($this->guid==""){
			$guid=$this->GetGUIDFromDB($_SERVER['REMOTE_ADDR'],$_SERVER['HTTP_HOST']);	
			//$log->general("GUID From DB=".$guid,1);
			if($guid==""){
				$guid=$this->SetGUID($this->make_guid());
				//$log->general("Make GUID".$guid,1);
			}
		}
		$this->guid=$guid;
		//$log->general("Set Look GUID->".$this->guid,1);
		//$log->general("\n",1);
		return $this->guid;
	}
	
	public function AddAllVariables(){
		//$log->general("Add All Vars",1);
		/*
		$gvr=array();
		...
		*/
		...
		//$this->var_save['SESSION']=$session_data;
		...
		//$log->general("Add All Vars->".var_export($this->var_save,true),3);
	}
	...
	public function GetGUIDFromDB($clientIP=false,$domainName=false){
		//$log->general("Get GUID From DB-> ".$clientIP."->".$domainName,1);
		...
		try{
			...
			////$log->general('GUID Success: $rslt->'.var_export($this->rslt,true));
			if(isset($this->rslt)){
				//$log->general("\n",1);
				$nrows=$this->r->NumRows();
				//$log->general("Rows-> ".$nrows."-",1);
				
				if($nrows>0){
					//print("exit-data");
					//$log->general("Get GUID From DB after query-> ".$query,1);
					while($data=$this->r->Fetch_Array()){
						$guid=$data[0];
						$Refresh_Time=$data[1];
						$domainsID=$data[2];	
						$siteuserclientID=$data[3];
						//$log->general('Data Success: $data->'.var_export($data,true));
					}
				}else{
					$guid="";
					$Refresh_Time=0;
			...
			//$log->general("MySQL Exception->".var_export($e,true)." ".$query);
		}
	}
	...
	public function GetDomainsID($domainName){
		...
		try{
			$this->rslt=$this->r->rawQuery($query);
			//$log->general('DomainsID Success: $rslt->'.var_export($this->rslt,true));
			if(isset($this->rslt)){
				if($this->r->NumRows()>0){
			...
			//$log->general("domainsID MySQL Exception->".var_export($e,true)." ".$query);
			return false;
		}
	}
	...
	public function Set_Database($domainsID=false,$guid=false){
		...
		////$log->general("Set_Database vs->");
		////$log->general($this->var_save);
		...
		//$log->general("Set Variables Database->".$query);
