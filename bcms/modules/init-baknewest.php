<?
	...
	if(isset($domain_user_data["db"])>0){
		if(count($domain_user_data["db"])>0){
			...
			if($r->NumRows($rslt)>0){
				...
				$sql="SELECT * FROM mod_business_categories WHERE id=".$domain_user_data["db"]['mod_business_categoriesID'];
			
				$rslt=$r->RawQuery($sql);
				$bizcat_data["db"]=$r->Fetch_Assoc(rslt);	
				
				$content_data["db"]['Meta_Title']=$domain_user["db"]['name']." - ".$bizcat_data["db"]['CategoryTitle']." - ".$content_data["db"]['Meta_Title'];
			}
		}
	}
	...
	while($csearch){
		...
		/*
		$sql="SELECT * FROM content_pages WHERE (domainsID=".DOMAINSID." OR domainsID=0)";
		$log->general("dd-Content sql-".$sql,1);
		$sql_home="SELECT * FROM content_pages WHERE HomePage='Yes' AND (domainsID=".DOMAINSID." OR domainsID=0)";
		...
		*/
		...
		$sql="SELECT * FROM content_pages WHERE (domainsID=".$domain_data["db"]['id']." OR domainsID=0)";
		//echo"--771---------------------------------------------------------------------------\n";
		$log->general("dd-Content sql-".$sql,1);
		$sql_home="SELECT * FROM content_pages WHERE HomePage='Yes' AND (domainsID=".$domain_data["db"]['id']." OR domainsID=0)";
